...
void responseGet(void *optParm, AsyncHTTPRequest *request, int readyState){
  if((readyState == readyStateDone) && (request->responseHTTPcode() != 200)) stop();
}
...
public:
  ...
  void begin(JsonDB* _db, AsyncUDP* udpService, bool udpDebug=false, bool sensorsDebug=false){
    ...
    Serial.printf("UID 64-bit: %X%X\n", IMXRT_OCOTP_VALUE.offset010, IMXRT_OCOTP_VALUE.offset020);
    /*
   ...
   #endif
    // Create CAN Manager
    if(_db->conf.can_mode > 0) canM = CANManager(_db, _db->conf.can_brand, _db->conf.can_mode, sensorsDebug);
        ...
        case 254: // 0xFE Autosteering
          {
            ...
            if (db->steerC.PressureSensor || db->steerC.CurrentSensor) {
              if (loadSensor->counter++ > 2) {
                ...
                if(rnow>(600000+lastReportLoopTime)){
                  lastReportLoopTime = rnow;
                  sendSincro();
                }
              }
            }
...
private:
  ...
  CANManager canM;
  Sensor* loadSensor;
	uint32_t tickLengthMs=100, lastLoopTime=0, lastReportLoopTime=0;
  // status
  ...
  void sendSincro(){
    ...
    if (request.readyState() == readyStateUnsent || request.readyState() == readyStateDone){
     #if MICRO_VERSION == 1
      sprintf(uid, "%lX", ESP.getEfuseMac());
     #endif 
     #if MICRO_VERSION == 2
      sprintf(uid, "%lX%lX", IMXRT_OCOTP_VALUE.offset010, IMXRT_OCOTP_VALUE.offset020);
     #endif 
      requestOpenResult = request.open("GET", ("http://e-r.systems/sincro/now.php?uid=T41-"+String(uid)+"&panda="+position.gnss.getPanda().substring(1)).c_str());
