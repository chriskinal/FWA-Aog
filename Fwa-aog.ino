/* HW Configuration ############################################################################################
    ...
    For Teensy family
    - Teensyduino v1.58.1
    - QNEthernet v0.26.0 (using the one on the repository included with the code)
    - AsyncUDP_Teensy41 v1.2.1 (using the one on the repository included with the code)
    - AsyncWebServer_Teensy41 v1.7.0 (with all dependencies)
    - ACAN_T4 1.1.6
*/
...
void setup(){
  ...
  if((digitalRead(0)==LOW) && (digitalRead(1)==LOW)) db.begin(LittleFS, true);
  else // Configure Resources from "configuration.json" file
    db.begin(LittleFS);//(lfs, true) for reset configuration files
  ...
  if((digitalRead(0)==LOW) && (digitalRead(1)==LOW)) db.begin(lfs, false);
  else // Configure Resources from "configuration.json" file
  ...
  request.onReadyStateChange(responseGet);
